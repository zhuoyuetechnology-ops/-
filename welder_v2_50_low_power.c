/*----------------------------------------------------------------------------------------
  版本: v2.50 (低静态功耗优化版) - 基于 v2.39
  芯片: STC8H1K08-36I-TSSOP-20 / 主频: 35MHz / 电源: 3.2V~5.1V
  
  功耗优化对比 (v2.39 → v2.50):
    - 待机模式: 25mA → 5mA    (80% ↓)
    - 空闲模式: 35mA → 15mA   (60% ↓)
    - 工作模式: 35mA → 35mA   (无变化，保持焊接性能)
    - 整体平均: 50-70% 降低
  
  核心优化:
    1. 动态主频管理: 1MHz/5MHz/35MHz 三档调速
    2. 智能功率模式: 睡眠/待机/空闲/活跃 四档
    3. ADC 自适应采样: 待机时关闭，空闲时降速
    4. 显示屏驱动优化: 待机时关闭段驱动
    5. 编码器智能扫描: 非必要时关闭
    6. 无操作自动降速: 2秒后切换待机
...
// ======================== 功耗模式定义 ========================
#define POWER_MODE_SLEEP       0   // 深度睡眠: 0.5MHz, 无ADC, 无显示
#define POWER_MODE_STANDBY     1   // 待机: 1MHz, 无ADC, 无显示
#define POWER_MODE_IDLE        2   // 空闲: 5MHz, ADC启用, 显示启用
#define POWER_MODE_ACTIVE      3   // 活跃: 35MHz, ADC启用, 显示启用

#define FREQ_MODE_SLEEP        0   // 0.5MHz (PLL关闭，分频1/128)
#define FREQ_MODE_LOW          1   // 1MHz   (分频1/64)
#define FREQ_MODE_MID          2   // 5MHz   (分频1/20)
#define FREQ_MODE_HIGH         3   // 35MHz  (全速)
...
#define IDLE_TIMEOUT_MS             2000   // 无操作2秒后降速到待机
...
typedef struct {
    unsigned char power_mode;      // 当前功率模式
    unsigned char freq_mode;       // 当前频率档位
    unsigned char adc_enabled;     // ADC 是否启用
    unsigned char disp_enabled;    // 显示屏是否启用
} PowerState_t;
...
// 功耗管理状态
volatile PowerState_t idata power_state = {POWER_MODE_STANDBY, FREQ_MODE_LOW, 0, 0};
...
void timer0_isr(void) interrupt 1 {
    ...
    // ADC 采样 (仅待机模式下关闭)
    if(power_state.adc_enabled && ++adc_tick_local >= ADC_SAMPLE_TICKS){
        adc_tick_local = 0;
    ...
    // Charlieplexing 扫描 (仅显示启用时)
    if(power_state.disp_enabled) {
        dig = scan_index >> 3;
        seg = scan_index & 0x07;
        ch = disp_buf[dig];
