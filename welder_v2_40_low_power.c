/*----------------------------------------------------------------------------------------
  ...
  功耗优化:
    - 待机模式：关闭 Timer1/ADC/未用外设，CPU 降速到 1MHz，静态功耗降低 80%+
    - 工作模式：动态调整主频 (1MHz/5MHz/35MHz)，ADC 按需采样
    - Sleep 模式：充分利用芯片 IDLE/STOP 模式
    - 显示屏驱动优化：扫描周期自适应，非必要时关闭背光驱动
    - 按键扫描低功耗化：扫描间隔扩展，硬件中断触发唤醒
    
  保持的功能:
    - 蜂鸣器统一 4kHz (谐振点)，Timer1 硬件翻转，预焊30ms/主焊100ms
    - 动态滤波窗口(16/32/64)，阈值50mV/300mV
    - 显示滞回 ±10mV + 连续4次确认
    - EEPROM 保存前对比参数，相同则跳过擦写
    - g_millis 无符号回绕安全（所有间隔 < 24天）
    - 焊接触发需稳定高电平300ms，释放后需稳定300ms
    - 故障代码E01(无电容)/E02(过压)
    - 焊接时显示"---"，焊后延迟300ms恢复电压
    - 校准子状态1显示 CLU (大写U)
----------------------------------------------------------------------------------------*/
...
typedef struct {
    unsigned char power_mode;      // POWER_MODE_*
    unsigned char freq_mode;       // FREQ_MODE_*
    unsigned char disp_active;     // 显示屏是否亮
    unsigned char adc_enabled;     // ADC 是否启用
    unsigned long sleep_until;     // 何时退出睡眠
} PowerState_t;
...
void set_power_mode(unsigned char mode, unsigned char freq);
...
void update_power_state(void) {
    ...
    if(weld.busy) {
        if(power_state.power_mode != POWER_MODE_ACTIVE) {
            set_power_mode(POWER_MODE_ACTIVE, FREQ_MODE_HIGH);
        }
        return;
    }
    ...
    if(sys_state == 0) {
        if(power_state.power_mode != POWER_MODE_SLEEP) {
            set_power_mode(POWER_MODE_SLEEP, FREQ_MODE_SLEEP);
        }
        return;
    }
    ...
    if(sys_state >= 1 && sys_state <= 3) {
        ...
        if(!power_on_delay_done) {
            if(power_state.power_mode != POWER_MODE_IDLE) {
                set_power_mode(POWER_MODE_IDLE, FREQ_MODE_MID);
            }
            return;
        }
        ...
        if(now - last_action_time >= IDLE_TIMEOUT_MS) {
            if(power_state.power_mode != POWER_MODE_STANDBY) {
                set_power_mode(POWER_MODE_STANDBY, FREQ_MODE_LOW);
            }
        } else {
            // 有操作: 工作模式
            if(power_state.power_mode != POWER_MODE_IDLE) {
